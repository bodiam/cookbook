...
${net.javaisp.cookbook.Product.list()}
  ...
  <g:each in="${net.javaisp.cookbook.Product.list()}" status="i" var="product">
    <img src="${createLinkTo(dir:'images/ingredients',file:product.name.toLowerCase() + '.jpg')}" id="${fieldValue(bean:product, field:'id')}" class="product" width="75" style="border: 1px #ddd solid" title="${fieldValue(bean:product, field:'name')}" /> 
    <a href="#" id="${fieldValue(bean:product, field:'id')}" class="product" />${fieldValue(bean:product, field:'name')}
  </g:each>
  </ul>
